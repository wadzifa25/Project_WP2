...
class Connect_db{
    ...
    // Order Tiket
    function set_Pesanan($id_tiket, $order_id, $email, $catatan, $rek_pengirim, $namarek_pengirim, $bank_pengirim, $nama_event, $bank_penerima, $total_pembayaran, $waktu_pesanan,$bukti_pembayaran, $status, $link_status, $warna_status){
        $sql = "INSERT INTO user_order(id,id_tiket ,order_id, email, catatan, rek_pengirim, namarek_pengirim, bank_pengirim, nama_event, bank_penerima, total_pembayaran, waktu_pesanan,bukti_pembayaran, status, link_status, warna_status)
               VALUES
               (NULL, '$id_tiket', '$order_id', '$email', '$catatan', '$rek_pengirim', '$namarek_pengirim', '$bank_pengirim', '$nama_event', '$bank_penerima', '$total_pembayaran', '$waktu_pesanan', '$bukti_pembayaran', '$status', '$link_status', '$warna_status') 
        ";
        if(mysqli_query($this->connect_db, $sql)){
            echo '<script>';
            echo 'alert("Pesanan Berhasil Dibuat, Silahkan Tunggu Admin Untuk Konfirmasi, Jika Terdapat Kendala Bisa Hubungi 0821-5978-1404")';
            echo '</script>';
        }
    }
